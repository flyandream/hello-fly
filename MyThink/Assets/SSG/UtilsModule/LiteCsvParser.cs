...
/// </summary>
	public class CsvFileReader : CsvFileCommon, IDisposable
	{
        ...
        public static DataTable ReadAll( MemoryStream stream, Encoding encoding, int scanRows )
        {
            using (var sr = new StreamReader(stream, encoding))
            {
                var cfr = new CsvFileReader(sr);
                List<List<string>> dataGrid = new List<List<string>>();
                if ( cfr.ReadAll( dataGrid, scanRows ))
                {
		...
		public bool ReadAll(List<List<string>> dataGrid, int scanRows = 0 ) {
